...
```mermaid
flowchart TD
    A[approval surfaces\npatch / tool / network / MCP / delegated] --> B[guardian runtime entry]
    B --> C[reviewer session manager\ntrunk / fork / snapshot]
    C --> D[guardian decision\nApproved / Denied / TimedOut / Aborted]
    D --> E[execution chain write-back]
    D --> F[protocol & UI projection\nstarted / completed / rationale / status]
    D -. reserved .-> G[analytics emission\ncustom fact -> reducer -> event]
```
